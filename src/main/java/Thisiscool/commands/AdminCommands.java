...
public class AdminCommands {
    ...
    public static void load() {
        Commands.admin("a")
                .register((args, player) -> Translator.translate(Player::admin, player, args[0], "commands.a.chat",
                        player.coloredName()));
        ...
        Commands.admin("core")
                ...
                .register((args, player) -> {
                    ...
                    if (notFoundCore(player, core))
                        return;

                    var team = args.length > 1 ? Find.team(args[1]) : player.team();
                    if (notFound(player, team))
                        return;

                    Call.constructFinish(player.tileOn(), core, player.unit(), (byte) 0, team, null);
                    Bundle.send(player, player.blockOn() == core ? "commands.core.success" : "commands.core.failed",
                            core.emoji(), core.name, team.coloredName());
                });
        ...
        Commands.admin("unit")
                ...
                .register((args, player) -> {
                    ...
                    if (target != player)
                        Bundle.send(player, "commands.unit.success.player", target.coloredName(), type.emoji(),
                                type.name);
                });
        ...
        Commands.admin("effect")
                ...
                .register((args, player) -> {
                    ...
                    if (notFound(player, effect))
                        return;

                    int duration = args.length > 1 ? Strings.parseInt(args[1]) : 60;
                    if (invalidDuration(player, duration, 0, maxEffectDuration))
                        return;
                    ...
                    if (duration > 0)
                        target.unit().apply(effect, duration * 60f);
                    else
                        target.unit().unapply(effect);

                    Bundle.send(target,
                            duration > 0 ? "commands.effect.apply.success" : "commands.effect.remove.success",
                            effect.emoji(), effect.name, duration);
                    if (target != player)
                        Bundle.send(player,
                                duration > 0 ? "commands.effect.apply.success.player"
                                        : "commands.effect.remove.success.player",
                                target.coloredName(), effect.emoji(), effect.name, duration);
                });
        ...
        Commands.admin("give")
                ...
                .register((args, player) -> {
                    ...
                    if (notFound(player, item))
                        return;

                    int amount = args.length > 1 ? Strings.parseInt(args[1]) : 1;
                    if (invalidAmount(player, amount, 1, maxGiveAmount))
                        return;

                    var team = args.length > 2 ? Find.team(args[2]) : player.team();
                    if (notFound(player, team) || noCores(player, team))
                        return;
        ...
        Commands.admin("spawn")
                ...
                .register((args, player) -> {
                    ...
                    int amount = args.length > 1 ? Strings.parseInt(args[1]) : 1;
                    if (invalidAmount(player, amount, 1, maxSpawnAmount))
                        return;
        ...
        Commands.admin("tp")
                ...
                .register((args, player) -> {
                    ...
                    if (invalidCoordinates(player, x, y))
                        return;
                    ...
                    for (int x = player.tileX(); x < player.tileX() + width; x += block.size)
                        for (int y = player.tileY(); y < player.tileY() + height; y += block.size) {
                            ...
                            if (block.isAir())
                                ...
                                tile.setFloorNet(tile.floor(), block);
                            else if (block.isFloor())
                                tile.setFloorNet(block, tile.overlay());
                            else
